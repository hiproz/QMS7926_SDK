...
//{
//    osal_start_timerEx(bleSmartPeripheral_TaskID, TIMER_BATT_EVT, intval_ms);
//    return 0;
//}
...
static int batt_adc_init(void)
{
  ...
  ret = hal_adc_config_channel(ADC_CH3P, cfg, batt_adc_evt);
  if(ret)
    return ret;
...
static void batt_charge_evt_hdl(GPIO_Pin_e pin, IO_Wakeup_Pol_e type)
{
  LOG("batt_charge_evt_hdl %d\n", type);
    ...
    if(s_batt_ctx.is_charging == FALSE){
      ...
      osal_set_event(bleSmartPeripheral_TaskID, BATT_CHARGE_EVT);
    }
	}
  else
	{
