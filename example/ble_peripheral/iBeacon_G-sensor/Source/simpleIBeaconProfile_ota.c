...
static uint8 simpleProfileChar6UserDesp[] = "G_sensor_rawdata\0";
...
static uint8 simpleProfileChar8Props = GATT_PROP_READ | GATT_PROP_NOTIFY; 

// Characteristic 8 Value
uint8 simpleProfileChar8[20];

// Simple Profile Characteristic 8 User Description
static uint8 simpleProfileChar8UserDesp[] = "G_sensor_stepdata\0";
// Simple Profile Characteristic 1 Configuration Each client has its own
// instantiation of the Client Characteristic Configuration. Reads of the
// Client Characteristic Configuration only shows the configuration for
// that client and writes only affect the configuration of that client.
...
static gattAttribute_t simpleProfileAttrTbl[SERVAPP_NUM_ATTR_SUPPORTED] = 
{
  ...
  { 
	...
	// ----------------------------------------------------------------------
    // Characteristic 8 Declaration, NOTify
    { 
        { ATT_BT_UUID_SIZE, characterUUID },
        GATT_PERMIT_READ, 
        0,
...
bStatus_t SimpleProfile_SetParameter( uint8 param, uint8 len, void *value )
{
  ...
  switch ( param )
  {
	...
	case SIMPLEPROFILE_CHAR8:
      if ( len <= 20 )   
      {
        osal_memcpy(simpleProfileChar8, value, len);				
      }
      else
      {
        ret = bleInvalidRange;
...
bStatus_t simpleProfile_Notify( uint8 param, uint8 len, void *value )
{
    ...
    {  
	  case SIMPLEPROFILE_CHAR6:
		...
		case SIMPLEPROFILE_CHAR8:
	    notfEnable = GATTServApp_ReadCharCfg( 0, simpleProfileChar8Config );
        ...
        if ( notfEnable & GATT_CLIENT_CFG_NOTIFY )
        {
          VOID osal_memcpy( simpleProfileChar8, value, len );
            ...
            ret=GATTServApp_ProcessCharCfg( simpleProfileChar6Config, simpleProfileChar8, FALSE,
                                    simpleProfileAttrTbl, GATT_NUM_ATTRS( simpleProfileAttrTbl ),
                                    INVALID_TASK_ID );      
