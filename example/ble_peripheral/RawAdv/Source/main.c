...
extern uint8 RawAdv_TaskID;
extern uint32_t osal_sys_tick;
extern uint8 advertData[];

uint8_t rx_length=0;

static void uart_tst_evt(uart_Evt_t* pev)
{
    rx_length += pev->len;
    if(rx_length >= MAX_ADVERT_LENGTH){
        if(rx_length >= MAX_ADVERT_LENGTH){
            for(uint8_t i=0;i<(MAX_ADVERT_LENGTH-(rx_length-pev->len));i++){
                advertData[1 + rx_length - pev->len+i] = pev->data[i];
            }
        }else if(pev->type==UART_EVT_TYPE_RX_DATA_TO){
            for(uint8_t i=0;i<pev->len;i++){
                advertData[1 + rx_length - pev->len+i] = pev->data[i];
            }
        }
        osal_stop_timerEx(RawAdv_TaskID,RAWADV_TIMEOUT_EVT);
        hal_pwrmgr_unlock(MOD_GPIO);      
        osal_set_event(RawAdv_TaskID,RAWADV_START_ADV );
        
    }else if(pev->type==UART_EVT_TYPE_RX_DATA){
        for(uint8_t i=0;i<pev->len;i++){
            advertData[1 + rx_length - pev->len+i] = pev->data[i];
        }
    }
...
static void gpio_pos_wakeuphandler(GPIO_Pin_e pin,IO_Wakeup_Pol_e type){
	if(type==POSEDGE){
    	rx_length=0;
    	for(uint8_t i=0;i<MAX_ADVERT_LENGTH;i++){
        	advertData[i+1]=0;
    	}
		hal_pwrmgr_lock(MOD_GPIO);
		hal_gpio_write(P15, 1);
        pGlobal_config[MAX_SLEEP_TIME] = 1000000;
...
static void hal_init(void)
{
    ...
    uart_Cfg_t cfg = {    
        ...
        .use_tx_buf = FALSE,
        .parity = FALSE,
        .evt_handler = uart_tst_evt,
    };
    hal_uart_init(cfg);
    hal_gpio_init();
    hal_pwrmgr_register(MOD_USR0, NULL, rf_wakeup_handler);
	hal_gpioin_register(P23,gpio_pos_wakeuphandler,NULL);
