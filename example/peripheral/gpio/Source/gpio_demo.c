...
static void key_press_evt(uint8_t i,key_evt_t key_evt)
{
	switch(key_evt)
	{
		...
		case KEY_EVT_LONG_RELEASE:
			osal_set_event(key_TaskID, key_evt);
			key_state.temp[i].info_inter |= key_evt;
			break;
...
//	key_state.key[1].pin = GPIO_P10;
	...
	key_state.key[0].pin = GPIO_P12;
	key_state.key[1].pin = GPIO_P13;
	
	for(i = 0; i < KEY_NUM; ++i){
		key_state.key[i].state = STATE_KEY_IDLE;
		key_state.key[i].idle_level = HIGH_IDLE;
