...
static hal_spi_t lcd_spi_handle = {SPI1};
...
#define X_OFFSET 0
#define Y_OFFSET 24
static void set_region(uint8_t x_start, uint8_t y_start, uint8_t x_end,
                       uint8_t y_end) {
  /*write_cmd(0x2a);
  //hal_gpio_write(DC_PIN,1);
  write_data(0);
...
static uint8_t s_framebuf[FRAME_BUF_SIZE] = {0};

int lcd_setscn_img(int index) {
  //  uint16_t i,j;
  uint32_t size = 115200; // 24*240*2 的总字节数
  ...
  while (size) {
    uint16_t size_buf = size > FRAME_BUF_SIZE ? FRAME_BUF_SIZE : size;
    // uint16_t size_buf = size > 8 ? 8 : size;
    size -= size_buf;
    osal_memcpy(s_framebuf, image_addr, size_buf);
    // osal_memset( s_framebuf, index, size_buf );
    /*
    for(i = 0; i< 128; i++){
    uint16_t tmp;
tmp = pbuf_u16[i];
...
pbuf_u16[i] = tmp;
    ...
    */
    ...
    // hal_spi_tx_frames_ignore_rx_polling(&lcd_spi_handle,
    // (uint8_t*)s_framebuf, size_buf);
...
int lcd_setscn_TFT(uint8_t x, uint8_t y, uint8_t w, uint8_t h, uint16_t color) {
  ...
  while (size) {
    ...
    // cs=true,polling mode
    //			hal_spi_tx_frames_ignore_rx_polling(&lcd_spi_handle,
    //(uint8_t*)s_framebuf, size_buf); 			lcd_spi_handle.force_cs
    //= true; 			lcd_spi_handle.polling_int = SPI_POLLING_MODE;
    //			if(hal_spi_get_transmit_bus_state(&lcd_spi_handle) ==
    // true)
    //			hal_spi_transmit(&lcd_spi_handle,(uint8_t*)s_framebuf,NULL,size_buf);
...
int lcd_draw_TFT(uint8_t x, uint8_t y, uint8_t w, uint8_t h,
                 const uint16_t *data) {
  uint8_t i, j;
  uint8_t xe = x + w - 1, ye = y + h - 1;
  uint16_t val;
...
int lcd_bus_init(void) {
  ...
  hal_gpio_pin_init(cfg.sclk_pin, OEN);
  ...
  hal_gpio_DS_control(cfg.ssn_pin, Bit_ENABLE);
...
int lcd_bus_deinit(void) { return hal_spi_bus_deinit(&lcd_spi_handle); }

int lcd_init(void) {
  uint16 color = RGB_Crimson;
  lcd_bus_init();
  hal_gpio_write(DC_PIN, 1);
  lcd_config();
  hal_gpio_write(DC_PIN, 0);
