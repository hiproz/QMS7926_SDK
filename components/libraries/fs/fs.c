...
static uint32_t spif_read(uint32_t addr)
{
	if ((addr < fs_offset_address)|| (addr >= (fs_offset_address + fs_sector_num*4096)))
		return PPlus_ERR_FS_PARAMETER;
	else
		return addr; 	
}
...
static int fs_search_items(search_type type,uint32_t* para1,uint32_t* para2)
{
	...
	for(m = 1;m < fs_sector_num;m++)
	{
		...
		{			
			...
			osal_memcpy((uint8_t*)(&i1),(uint8_t*)(spif_read(FS_ABSOLUTE_ADDR(ab_addr))),FS_ITEM_HEAD_LEN);
			switch(type)
			{
...
static int fs_init(void)
{
	...
	{
		osal_memcpy((uint8_t*)(&flash_rd_cfg),(uint8_t*)(spif_read(FS_ABSOLUTE_ADDR(4096*i))),sizeof(fs_cfg_t));
...
int hal_fs_item_read(uint16_t id,uint8_t* buf,uint16_t buf_len,uint16_t* len)
{
	...
	if(hal_fs_item_find_id(id,&addr) == PPlus_SUCCESS)
	{	
		osal_memcpy((uint8_t*)(&i1),(uint8_t*)(spif_read(FS_ABSOLUTE_ADDR(addr))),FS_ITEM_HEAD_LEN);
		if(len != NULL){
			*len = i1.b.len;
		}
		temp_len = i1.b.len;
		...
		if(buf_len >= i1.b.len)
		{
			while(temp_len > 0)
			{
				...
				osal_memcpy((buf + i), (uint8_t*)spif_read(FS_ABSOLUTE_ADDR(addr + FS_ITEM_HEAD_LEN)),rd_len);
				temp_len -= rd_len;
...
int hal_fs_garbage_collect(void)
{
	...
	for(i = 0;i < (fs.cfg.sector_num - 1);i++)
	{
		...
		{
				osal_memcpy((uint8_t*)(&i1),(uint8_t*)(spif_read(FS_ABSOLUTE_ADDR(addr_rd))),FS_ITEM_HEAD_LEN);
			
				if(i1.b.pro == ITEM_USED)
				{
						osal_memcpy(buf, (uint8_t*)spif_read(FS_ABSOLUTE_ADDR(addr_rd + FS_ITEM_HEAD_LEN)),FS_ITEM_DATA_LEN);
