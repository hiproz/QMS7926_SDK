...
uint8_t ota_patition_buffer[80*1024] __attribute__((section("ota_partition_buffer_area")));
...
bool validate_partition_parameter(ota_part_t* ppart)
{
    if(s_ota_ctx.ota_resource){
      ...
      if(ppart->run_addr < SRAM0_BASE_ADDRESS || ppart->run_addr > SRAM0_BASE_ADDRESS + 138*1024)
        return FALSE;
      if(ppart->flash_addr&OTAF_BASE_ADDR != OTAF_BASE_ADDR || ppart->flash_addr + ppart->size > OTAF_END_ADDR+1)
        return FALSE;
...
void process_ctrl_cmd(uint8_t* cmdbuf, uint8_t size){
  ...
  case OTA_CMD_PARTITION_INFO:
  {
    ...
    s_ota_ctx.partition_buf = ota_patition_buffer;
    osal_memset(ota_patition_buffer, 0xff, 80*1024);
...
/*  case OTA_CMD_BLOCK_INFO:
  ...
  case OTA_CMD_ERASE:
  {
    ...
    //erase
    ret = ota_flash_erase_resource(flash_addr, flash_size);
...
{
  uint32_t* prunpc = (uint32_t*)(0x1fff4000);
