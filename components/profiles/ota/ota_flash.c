...
int __attribute__((section("ota_app_loader_area"))) ota_flash_load_app(void)
{
  ...
  for(i = 1; i< partition_num+1; i++){
    ...
    //load binary
      ...
      //aes_ccm_QST_dec((const unsigned char*)0x200127e0, (uint8_t*)(flash_addr + bank_addr), (int)size, NULL, (uint8_t*)run_addr);
    }
    else
    {
...
int ota_flash_erase_resource(uint32_t flash_addr, uint32_t size)
{
