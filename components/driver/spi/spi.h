 ...
 *
 * polling mode:this function will return just after all the transmit has been
 *finished.
 *
 * int mode:the function just start the tranmsit,when tranmit finish the idle
 *flag will set true in int. so after call this api,you can use a callback
 *function to tell transmit result.
 *
 * force_cs=true:data will transmit in one time.
 *               cs is valid in the transmit.
 *
 * force_cs=false:data will transmit in several times.one time transmit one
 *frame.frame length is 8 bytes. cs is valid only when the frame transmit��
 *
 * input parameters
 * @param       spi_ptr:which spi module used,polling mode or int mode,force cs
 *manually or not
