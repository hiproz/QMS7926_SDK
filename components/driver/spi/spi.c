...
static void hal_spi_write_fifo(AP_SSI_TypeDef *Ssix, uint16_t len,
                               uint8_t *tx_rx_ptr) {
  uint16_t i = 0;
  HAL_ENTER_CRITICAL_SECTION();
  while (i < len) {
...
static void hal_spi_read_fifo(AP_SSI_TypeDef *Ssix, uint8_t len,
                              uint8_t *tx_rx_ptr) {
  uint8_t i = 0;
  HAL_ENTER_CRITICAL_SECTION();
  while (i < len) {
...
static void spi_int_callback(uint8_t spi_index) {
  ...
  if (((spi_index != 0) && (spi_index != 1)) ||
      (m_spiCtx[spi_index].spi_info == NULL))
    return;
 ...
 *
 * @brief       This function process for spi0 interrupt,when use int please
 *consummate its callbackfunction
 *
 * input parameters
 *
 * @param       None.
 ...
 * @return      None.
 **************************************************************************************/
void __attribute__((used)) hal_SPI0_IRQHandler(void) { spi_int_callback(0); }

/**************************************************************************************
 * @fn          hal_SPI1_IRQHandler
 *
 * @brief       This function process for spi1 interrupt,when use int please
 *consummate its callbackfunction
 *
 * input parameters
 *
 ...
 * @return      None.
 **************************************************************************************/
void __attribute__((used)) hal_SPI1_IRQHandler(void) { spi_int_callback(1); }

/**************************************************************************************
 * @fn          hal_spi_pin_init
 *
 * @brief       This function process for spi pin initial(4 lines);You can use
 *two spi,spi0 and spi1,should programe by USE_AP_SPIX
 *
 * input parameters
 *
 * @param       GPIO_Pin_e sck_pin: define sclk pin
 *              GPIO_Pin_e ssn_pin: define ssn pin
 *              GPIO_Pin_e tx_pin: define transmit pin;when use as master,it's
 *mosi pin;corresponding,use as slave,it's miso GPIO_Pin_e rx_pin: define
 *receive pin;when use as master,it's miso pin;corresponding,use as slave,it's
 *mosi
 *
 * output parameters
 *
 ...
 * @return      None.
 **************************************************************************************/
static void hal_spi_pin_init(hal_spi_t *spi_ptr, GPIO_Pin_e sck_pin,
                             GPIO_Pin_e ssn_pin, GPIO_Pin_e tx_pin,
                             GPIO_Pin_e rx_pin) {
...
static void hal_spi_pin_deinit(GPIO_Pin_e sck_pin, GPIO_Pin_e ssn_pin,
                               GPIO_Pin_e tx_pin, GPIO_Pin_e rx_pin) {
 ...
 *              SPI_SCMOD_e scmod: Serial Clock Polarity and Phase select;
 *SPI_MODE0,        //SCPOL=0,SCPH=0(default) SPI_MODE1,        //SCPOL=0,SCPH=1
 *SPI_MODE2, // SCPOL = 1, SCPH = 0 SPI_MODE3, // SCPOL=1,SCPH=1
 *SPI_TMOD_e tmod: Transfer Mode SPI_TRXD,                 // Transmit &
 *Receive(default) SPI_TXD,      // Transmit Only SPI_RXD,         //Receive
 *Only SPI_EEPROM,               // EEPROM Read
 **output parameters **@param None.**@ return None.*
 *************************************************************************************/
static void hal_spi_master_init(hal_spi_t *spi_ptr, uint32_t baud,
                                SPI_SCMOD_e scmod, SPI_TMOD_e tmod) {
  uint8_t shift = 0;
  AP_SSI_TypeDef *Ssix = AP_SPI0;
  uint16_t baud_temp;
 ...
 *SPI_TMOD_e tmod: Transfer *Mode SPI_TRXD,                 // Transmit &
 *Receive(default) SPI_TXD,      // Transmit Only SPI_RXD,         //Receive
 *Only SPI_EEPROM, //EEPROM Read output parameters *
 *@param None.*
 *@ return None.
 *************************************************************************************/
void hal_spi_slave_init(hal_spi_t *spi_ptr, uint32_t baud, SPI_SCMOD_e scmod,
                        SPI_TMOD_e tmod) {
  uint8_t shift = 0;
  AP_SSI_TypeDef *Ssix = AP_SPI0;
  uint16_t baud_temp;
 ...
 * input parameters
 *
 * @param       	spi_Cfg_t cfg:there are two parameter you need config to
 *use spi,one is cfg,another is the macro SPI_MASTER_MODE refer to struct
 *spi_Cfg_t,you will config it right.
 *
 * output parameters
 *
 * @param       None.
 *
 * @return
 * 							PPlus_SUCCESS:config
 *success. PPlus_ERR_BUSY:the spi you want to use is work,please use another or
 *stop it and reuse it. PPlus_ERR_INVALID_PARAM:there are two spi in mcu,you
...
int hal_spi_bus_deinit(hal_spi_t *spi_ptr) {
  ...
  hal_spi_pin_deinit(m_spiCtx[spi_ptr->spi_index].cfg.sclk_pin,
                     m_spiCtx[spi_ptr->spi_index].cfg.ssn_pin,
                     m_spiCtx[spi_ptr->spi_index].cfg.MOSI,
                     m_spiCtx[spi_ptr->spi_index].cfg.MISO);
 ...
 *
 * @brief       spi master sends commands and receives response,its length is
 *from 1 to 8.
 *
 * input parameters
 *
 ...
 *              PPlus_ERR_BUSY
 **************************************************************************************/
static int hal_spi_tx_rx_single_frame(hal_spi_t *spi_ptr, uint8_t *tx_buf,
...
int hal_spi_transmit(hal_spi_t *spi_ptr, uint8_t *tx_buf, uint8_t *rx_buf,
                     uint16_t len) {
  ...
  if (m_spiCtx[spi_ptr->spi_index].cfg.int_mode ==
      true) // set tx buf first when use int,use hal_spi_int_set_tx_buf
  {
    if ((m_spiCtx[spi_ptr->spi_index].transmit.tx_ptr == NULL) ||
        (len > m_spiCtx[spi_ptr->spi_index].transmit.tx_buf_len))
      return PPlus_ERR_INVALID_PARAM;
  }
  ...
  if (m_spiCtx[spi_ptr->spi_index].cfg.int_mode == false) {
    // while (len > 0) {
    //   temp = (len >= 64) ? 64 : len;
    //   hal_spi_tx_rx_single_frame(spi_ptr, (tx_buf + offset), (rx_buf +
    //   offset),
    //                              temp);
    ...
    // zcj
    hal_spi_tx_rx_single_frame(spi_ptr, tx_buf, rx_buf, len);
...
int hal_spi_int_set_tx_buf(hal_spi_t *spi_ptr, uint8_t *tx_buf, uint16_t len) {
  if ((spi_ptr == NULL) || (spi_ptr->spi_index > 1) || (tx_buf == NULL) ||
      (len == 0))
    return PPlus_ERR_INVALID_PARAM;
